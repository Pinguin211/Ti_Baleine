# Prompt — écrire la fiche de cas CASE-RES-402
...
> `docs/prompt/code/reservation/prompt-case-res-402.md`, qui fait
> écrire le test.
>
...
- Hypothèse non validée applicable à ce cas : la formulation textuelle exacte de
  la mention d'avertissement n'est pas validée par la direction. La fiche ne
  doit donc exiger que la *présence* de la mention, jamais son texte au mot
  près (voir specs/reservation.md, § « Ce qui n'est pas défini »).

TA TÂCHE
Écris UNE fiche de cas, et une seule : tests/cases/reservation/CASE-RES-402.md
...
2. En-tête imposé : Spécification `SPEC-RESERVATION-03` ; Critère
   d'acceptation `AC-7, AC-8` ; Type acceptation ; Niveau de risque élevé.
...
8. La checklist « Revue du test automatisé » se termine obligatoirement par
   ...
   `CASE_RES_402` », et « Aucune assertion étrangère à ce cas n'a été
   ajoutée ».
...
10. Ne modifie aucun fichier en dehors de :
    tests/cases/reservation/CASE-RES-402.md
