...
Commande pour ce seul test : npx vitest run tests/tests-unitaires/reservation/CASE-RES-404.test.ts -t "test_CASE_RES_404_bascule_bilingue_fr_en_conservation_donnees_tunnel_et_solde"
Commande d'audit d'architecture : npm run arch:report

CE QUE JE TE DONNE
- Le test à faire passer : tests/tests-unitaires/reservation/CASE-RES-404.test.ts::test_CASE_RES_404_bascule_bilingue_fr_en_conservation_donnees_tunnel_et_solde
- Le cas de test : tests/cases/reservation/CASE-RES-404.md
- La spécification : specs/reservation.md, section SPEC-RESERVATION-03 (AC-1, AC-10)
- Les exigences d'architecture : specs/architecture.md (SPEC-ARCH-01, SPEC-ARCH-02, SPEC-ARCH-03)
- Les couches du domaine que tu peux modifier : src/config/, src/utils/, src/schemas/

TA TÂCHE
1. Fais passer au vert le test test_CASE_RES_404_bascule_bilingue_fr_en_conservation_donnees_tunnel_et_solde, et lui seul.
...
4. La règle vit dans le domaine : dictionnaires d'objets de constantes pures statiques dans src/config/i18n.constants.ts (0 logique, 0 fonction) et utilitaires de formatage/conservation d'état dans src/utils/i18n.ts.
...
7. Ne modifie aucun fichier en dehors de : src/config/, src/utils/, src/schemas/.
