...
Commande pour ce seul test : npx vitest run tests/tests-unitaires/reservation/CASE-RES-403.test.ts -t "test_CASE_RES_403_reservation_privatisation_tikap_saint_leu_acompte_50_pourcent_jauge_bloquee"
Commande d'audit d'architecture : npm run arch:report

CE QUE JE TE DONNE
- Le test à faire passer : tests/tests-unitaires/reservation/CASE-RES-403.test.ts::test_CASE_RES_403_reservation_privatisation_tikap_saint_leu_acompte_50_pourcent_jauge_bloquee
- Le cas de test : tests/cases/reservation/CASE-RES-403.md
- La spécification : specs/reservation.md, section SPEC-RESERVATION-03 (AC-5, AC-8)
- Les exigences d'architecture : specs/architecture.md (SPEC-ARCH-01, SPEC-ARCH-02, SPEC-ARCH-03)
- Les couches du domaine que tu peux modifier : src/services/server/, src/utils/, src/actions/

TA TÂCHE
1. Fais passer au vert le test test_CASE_RES_403_reservation_privatisation_tikap_saint_leu_acompte_50_pourcent_jauge_bloquee, et lui seul.
...
4. La règle vit dans le domaine : forfait et acompte 50 % dans src/utils/pricing-calculator.ts, blocage de jauge dans src/services/server/booking-capacity.service.ts.
...
7. Ne modifie aucun fichier en dehors de : src/services/server/, src/utils/, src/actions/.
