...
Commande pour ce seul test : npx vitest run tests/tests-unitaires/reservation/CASE-RES-406.test.ts -t "test_CASE_RES_406_reservation_privatisation_grand_bleu_saint_gilles_acompte_50_pourcent"
Commande d'audit d'architecture : npm run arch:report

CE QUE JE TE DONNE
- Le test à faire passer : tests/tests-unitaires/reservation/CASE-RES-406.test.ts::test_CASE_RES_406_reservation_privatisation_grand_bleu_saint_gilles_acompte_50_pourcent
...
- Les couches du domaine que tu peux modifier : src/utils/, src/services/server/, src/actions/

TA TÂCHE
1. Fais passer au vert le test test_CASE_RES_406_reservation_privatisation_grand_bleu_saint_gilles_acompte_50_pourcent, et lui seul.
...
4. La règle vit dans le domaine : forfait Grand Bleu dans src/utils/pricing-calculator.ts et blocage de jauge dans src/services/server/booking-capacity.service.ts.
...
7. Ne modifie aucun fichier en dehors de : src/utils/, src/services/server/, src/actions/.
