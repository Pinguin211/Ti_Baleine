...
Commande pour ce seul test : npx vitest run tests/tests-unitaires/reservation/CASE-RES-402.test.ts -t "test_CASE_RES_402_reservation_creneau_sous_alerte_preannulation_meteo_mention_avertissement"
Commande d'audit d'architecture : npm run arch:report

CE QUE JE TE DONNE
- Le test à faire passer : tests/tests-unitaires/reservation/CASE-RES-402.test.ts::test_CASE_RES_402_reservation_creneau_sous_alerte_preannulation_meteo_mention_avertissement
- Le cas de test : tests/cases/reservation/CASE-RES-402.md
- La spécification : specs/reservation.md, section SPEC-RESERVATION-03 (AC-7, AC-8)
...
TA TÂCHE
1. Fais passer au vert le test test_CASE_RES_402_reservation_creneau_sous_alerte_preannulation_meteo_mention_avertissement, et lui seul.
...
4. La règle vit dans le domaine : src/services/server/booking-slot.service.ts et src/actions/booking.actions.ts. Pas dans un composant UI ni dans un hook.
