...
Commande pour ce seul test : npx vitest run tests/tests-unitaires/reservation/CASE-RES-416.test.ts -t "test_CASE_RES_416_echec_paiement_acompte_cb_aucune_reservation_jauge_intacte"
Commande d'audit d'architecture : npm run arch:report

CE QUE JE TE DONNE
- Le test à faire passer : tests/tests-unitaires/reservation/CASE-RES-416.test.ts::test_CASE_RES_416_echec_paiement_acompte_cb_aucune_reservation_jauge_intacte
- Le cas de test : tests/cases/reservation/CASE-RES-416.md
- La spécification : specs/reservation.md, section SPEC-RESERVATION-03 (AC-8)
...
TA TÂCHE
1. Fais passer au vert le test test_CASE_RES_416_echec_paiement_acompte_cb_aucune_reservation_jauge_intacte, et lui seul.
...
4. La règle vit dans le domaine : src/services/server/payment.service.ts et src/actions/booking.actions.ts.
