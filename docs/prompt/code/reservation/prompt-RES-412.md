...
Commande pour ce seul test : npx vitest run tests/tests-unitaires/reservation/CASE-RES-412.test.ts -t "test_CASE_RES_412_plafonnement_jauge_24_places_saint_gilles_mardi_jeudi_matin"
Commande d'audit d'architecture : npm run arch:report

CE QUE JE TE DONNE
- Le test à faire passer : tests/tests-unitaires/reservation/CASE-RES-412.test.ts::test_CASE_RES_412_plafonnement_jauge_24_places_saint_gilles_mardi_jeudi_matin
- Le cas de test : tests/cases/reservation/CASE-RES-412.md
- La spécification : specs/reservation.md, section SPEC-RESERVATION-03 (AC-2, AC-5)
...
TA TÂCHE
1. Fais passer au vert le test test_CASE_RES_412_plafonnement_jauge_24_places_saint_gilles_mardi_jeudi_matin, et lui seul.
...
4. La règle vit dans le domaine : src/services/server/booking-capacity.service.ts et src/services/server/booking-slot.service.ts.
