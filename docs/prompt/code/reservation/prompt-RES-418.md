...
Commande pour ce seul test : npx vitest run tests/tests-unitaires/reservation/CASE-RES-418.test.ts -t "test_CASE_RES_418_envoi_sms_j_moins_1_paiement_solde_en_ligne_statut_payee_completement"
Commande d'audit d'architecture : npm run arch:report

CE QUE JE TE DONNE
- Le test à faire passer : tests/tests-unitaires/reservation/CASE-RES-418.test.ts::test_CASE_RES_418_envoi_sms_j_moins_1_paiement_solde_en_ligne_statut_payee_completement
- Le cas de test : tests/cases/reservation/CASE-RES-418.md
- La spécification : specs/reservation.md, section SPEC-RESERVATION-03 (AC-9, AC-10)
- Les exigences d'architecture : specs/architecture.md (SPEC-ARCH-01, SPEC-ARCH-02, SPEC-ARCH-03)
- Les couches du domaine que tu peux modifier : src/services/server/, src/actions/

TA TÂCHE
1. Fais passer au vert le test test_CASE_RES_418_envoi_sms_j_moins_1_paiement_solde_en_ligne_statut_payee_completement, et lui seul.
...
4. La règle vit dans le domaine : src/services/server/sms-notification.service.ts et src/services/server/payment.service.ts.
...
6. Respecte strictement les spécifications de specs/architecture.md :
   ...
   - Flux d'imports & étanchéité : respect de la matrice modulaire (src/actions/ -> src/services/server/ autorisé, mais interdiction formelle à src/services/server/ d'importer src/actions/) et interdiction des modules serveurs/secrets dans le client et les hooks.
...
7. Ne modifie aucun fichier en dehors de : src/services/server/, src/actions/.
