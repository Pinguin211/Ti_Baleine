...
Commande pour ce seul test : npx vitest run tests/tests-unitaires/reservation/CASE-RES-407.test.ts -t "test_CASE_RES_407_rejet_formulaire_coordonnees_nom_prenom_email_manquant_ou_invalide"
Commande d'audit d'architecture : npm run arch:report

CE QUE JE TE DONNE
- Le test à faire passer : tests/tests-unitaires/reservation/CASE-RES-407.test.ts::test_CASE_RES_407_rejet_formulaire_coordonnees_nom_prenom_email_manquant_ou_invalide
- Le cas de test : tests/cases/reservation/CASE-RES-407.md
- La spécification : specs/reservation.md, section SPEC-RESERVATION-03 (AC-6)
...
TA TÂCHE
1. Fais passer au vert le test test_CASE_RES_407_rejet_formulaire_coordonnees_nom_prenom_email_manquant_ou_invalide, et lui seul.
...
4. La règle vit dans le domaine : src/schemas/validation/booking-contact.schema.ts.
...
8. Ne modifie aucun fichier en dehors de : src/schemas/validation/, src/utils/.
