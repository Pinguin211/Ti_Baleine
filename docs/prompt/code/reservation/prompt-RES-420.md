...
Commande pour ce seul test : npx vitest run tests/tests-unitaires/reservation/CASE-RES-420.test.ts -t "test_CASE_RES_420_acces_page_solde_token_expire_plus_de_1_heure_redirection_sur_place"
Commande d'audit d'architecture : npm run arch:report

CE QUE JE TE DONNE
- Le test à faire passer : tests/tests-unitaires/reservation/CASE-RES-420.test.ts::test_CASE_RES_420_acces_page_solde_token_expire_plus_de_1_heure_redirection_sur_place
- Le cas de test : tests/cases/reservation/CASE-RES-420.md
- La spécification : specs/reservation.md, section SPEC-RESERVATION-03 (AC-10)
...
- Les couches du domaine que tu peux modifier : src/services/server/, src/schemas/

TA TÂCHE
1. Fais passer au vert le test test_CASE_RES_420_acces_page_solde_token_expire_plus_de_1_heure_redirection_sur_place, et lui seul.
...
4. La règle vit dans le domaine : src/services/server/payment.service.ts.
...
8. Ne modifie aucun fichier en dehors de : src/services/server/, src/schemas/.
