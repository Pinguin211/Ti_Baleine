...
Commande pour ce seul test : npx vitest run tests/tests-unitaires/reservation/CASE-RES-421.test.ts -t "test_CASE_RES_421_non_utilisation_lien_solde_maintien_reservation_payee_partiellement"
Commande d'audit d'architecture : npm run arch:report

CE QUE JE TE DONNE
- Le test à faire passer : tests/tests-unitaires/reservation/CASE-RES-421.test.ts::test_CASE_RES_421_non_utilisation_lien_solde_maintien_reservation_payee_partiellement
- Le cas de test : tests/cases/reservation/CASE-RES-421.md
- La spécification : specs/reservation.md, section SPEC-RESERVATION-03 (AC-8, AC-10)
...
TA TÂCHE
1. Fais passer au vert le test test_CASE_RES_421_non_utilisation_lien_solde_maintien_reservation_payee_partiellement, et lui seul.
...
4. La règle vit dans le domaine : src/services/server/booking-capacity.service.ts et src/services/server/payment.service.ts.
