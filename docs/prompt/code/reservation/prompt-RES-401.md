...
Commande pour ce seul test : npx vitest run tests/tests-unitaires/reservation/CASE-RES-401.test.ts -t "test_CASE_RES_401_reservation_individuelle_saint_leu_majoration_acompte_payee_partiellement"
Commande d'audit d'architecture : npm run arch:report

CE QUE JE TE DONNE
- Le test à faire passer : tests/tests-unitaires/reservation/CASE-RES-401.test.ts::test_CASE_RES_401_reservation_individuelle_saint_leu_majoration_acompte_payee_partiellement
...
- Les couches du domaine que tu peux modifier : src/services/server/, src/utils/, src/schemas/

TA TÂCHE
1. Fais passer au vert le test test_CASE_RES_401_reservation_individuelle_saint_leu_majoration_acompte_payee_partiellement, et lui seul.
...
4. La règle vit dans l'arborescence imposée par SPEC-ARCH-02 : calculs de majoration dans src/utils/ et planning/jauges dans src/services/server/. Pas dans un contrôleur, pas dans un écran, pas dans une requête.
...
7. Ne modifie aucun fichier en dehors de : src/services/server/, src/utils/, src/schemas/.
