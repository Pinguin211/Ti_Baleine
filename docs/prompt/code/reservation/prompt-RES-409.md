...
Commande pour ce seul test : npx vitest run tests/tests-unitaires/reservation/CASE-RES-409.test.ts -t "test_CASE_RES_409_fermeture_annuelle_25_decembre_et_1er_janvier_aucun_creneau"
Commande d'audit d'architecture : npm run arch:report

CE QUE JE TE DONNE
- Le test à faire passer : tests/tests-unitaires/reservation/CASE-RES-409.test.ts::test_CASE_RES_409_fermeture_annuelle_25_decembre_et_1er_janvier_aucun_creneau
...
TA TÂCHE
1. Fais passer au vert le test test_CASE_RES_409_fermeture_annuelle_25_decembre_et_1er_janvier_aucun_creneau, et lui seul.
...
4. La règle vit dans le domaine : dates de fermeture dans src/config/booking.constants.ts et filtrage dans src/services/server/booking-slot.service.ts.
...
8. Ne modifie aucun fichier en dehors de : src/services/server/, src/config/, src/utils/.
