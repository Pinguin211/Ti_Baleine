...
Commande pour ce seul test : npx vitest run tests/tests-unitaires/facturation/CASE-FAC-718.test.ts -t "test_CASE_FAC_718_echec_envoi_smtp_passage_statut_echec_emission_acompte_ou_solde"
Commande d'audit d'architecture : npm run arch:report

CE QUE JE TE DONNE
- Le test à faire passer : tests/tests-unitaires/facturation/CASE-FAC-718.test.ts::test_CASE_FAC_718_echec_envoi_smtp_passage_statut_echec_emission_acompte_ou_solde
- Le cas de test : tests/cases/facturation/CASE-FAC-718.md
- La spécification : specs/facturation.md, section SPEC-FAC-02 (AC-4, AC-5, Cas limite #1)
...
TA TÂCHE
1. Fais passer au vert le test test_CASE_FAC_718_echec_envoi_smtp_passage_statut_echec_emission_acompte_ou_solde, et lui seul.
...
4. La règle vit dans le domaine : src/services/server/envoyer-courriel-facture.service.ts (interception de l'échec) et src/services/server/enregistrer-statut-emission-facture.service.ts (statut d'échec horodaté).
