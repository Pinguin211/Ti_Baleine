...
Commande pour ce seul test : npx vitest run tests/tests-unitaires/facturation/CASE-FAC-711.test.ts -t "test_CASE_FAC_711_mention_explicite_port_embarquement_sur_factures_acompte_et_solde"
Commande d'audit d'architecture : npm run arch:report

CE QUE JE TE DONNE
- Le test à faire passer : tests/tests-unitaires/facturation/CASE-FAC-711.test.ts::test_CASE_FAC_711_mention_explicite_port_embarquement_sur_factures_acompte_et_solde
- Le cas de test : tests/cases/facturation/CASE-FAC-711.md
- La spécification : specs/facturation.md, section SPEC-FAC-02 (AC-3, Scénario 1)
...
TA TÂCHE
1. Fais passer au vert le test test_CASE_FAC_711_mention_explicite_port_embarquement_sur_factures_acompte_et_solde, et lui seul.
...
4. La règle vit dans le domaine : src/services/server/generer-facture-acompte-pdf.service.ts et src/services/server/generer-facture-solde-pdf.service.ts (report du port d'embarquement).
