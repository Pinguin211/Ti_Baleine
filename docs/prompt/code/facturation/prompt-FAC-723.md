...
Commande pour ce seul test : npx vitest run tests/tests-unitaires/facturation/CASE-FAC-723.test.ts -t "test_CASE_FAC_723_traitement_idempotent_notification_paiement_double_acompte_et_solde_independants"
Commande d'audit d'architecture : npm run arch:report

CE QUE JE TE DONNE
- Le test à faire passer : tests/tests-unitaires/facturation/CASE-FAC-723.test.ts::test_CASE_FAC_723_traitement_idempotent_notification_paiement_double_acompte_et_solde_independants
- Le cas de test : tests/cases/facturation/CASE-FAC-723.md
- La spécification : specs/facturation.md, section SPEC-FAC-02 (AC-8, Cas limite #4)
...
TA TÂCHE
1. Fais passer au vert le test test_CASE_FAC_723_traitement_idempotent_notification_paiement_double_acompte_et_solde_independants, et lui seul.
...
4. La règle vit dans le domaine : src/services/server/verifier-idempotence-emission-facture.service.ts, utilisé indépendamment par src/actions/emettre-facture-acompte-apres-paiement.ts et src/actions/emettre-facture-solde-apres-paiement.ts.
