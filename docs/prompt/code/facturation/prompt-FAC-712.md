...
Commande pour ce seul test : npx vitest run tests/tests-unitaires/facturation/CASE-FAC-712.test.ts -t "test_CASE_FAC_712_ligne_detaillee_supplement_saint_leu_facture_acompte_et_solde"
Commande d'audit d'architecture : npm run arch:report

CE QUE JE TE DONNE
- Le test à faire passer : tests/tests-unitaires/facturation/CASE-FAC-712.test.ts::test_CASE_FAC_712_ligne_detaillee_supplement_saint_leu_facture_acompte_et_solde
- Le cas de test : tests/cases/facturation/CASE-FAC-712.md
- La spécification : specs/facturation.md, section SPEC-FAC-02 (AC-1, AC-2, AC-3, Scénario 1, Cas limite #5)
- Les exigences d'architecture : specs/architecture.md (SPEC-ARCH-01, SPEC-ARCH-02, SPEC-ARCH-03)
- Les couches du domaine que tu peux modifier : src/services/server/, src/utils/

TA TÂCHE
1. Fais passer au vert le test test_CASE_FAC_712_ligne_detaillee_supplement_saint_leu_facture_acompte_et_solde, et lui seul.
...
4. La règle vit dans le domaine : src/services/server/generer-facture-acompte-pdf.service.ts, src/services/server/generer-facture-solde-pdf.service.ts (ligne dédiée au supplément) et src/utils/calcul-tarification-facturation.util.ts (montant du supplément).
...
7. Ne modifie aucun fichier en dehors de : src/services/server/, src/utils/
