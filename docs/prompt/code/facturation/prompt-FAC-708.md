...
Commande pour ce seul test : npx vitest run tests/tests-unitaires/facturation/CASE-FAC-708.test.ts -t "test_CASE_FAC_708_mentions_acompte_acquitte_et_acquittee_montants_ttc_sur_pdf"
Commande d'audit d'architecture : npm run arch:report

CE QUE JE TE DONNE
- Le test à faire passer : tests/tests-unitaires/facturation/CASE-FAC-708.test.ts::test_CASE_FAC_708_mentions_acompte_acquitte_et_acquittee_montants_ttc_sur_pdf
- Le cas de test : tests/cases/facturation/CASE-FAC-708.md
- La spécification : specs/facturation.md, section SPEC-FAC-02 (AC-1, AC-2, Règle)
...
TA TÂCHE
1. Fais passer au vert le test test_CASE_FAC_708_mentions_acompte_acquitte_et_acquittee_montants_ttc_sur_pdf, et lui seul.
...
4. La règle vit dans le domaine : src/services/server/generer-facture-acompte-pdf.service.ts et src/services/server/generer-facture-solde-pdf.service.ts (mentions explicites et montants).
