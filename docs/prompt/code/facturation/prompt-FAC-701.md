...
Commande pour ce seul test : npx vitest run tests/tests-unitaires/facturation/CASE-FAC-701.test.ts -t "test_CASE_FAC_701_emission_facture_acompte_puis_solde_privatisation_forfaitaire"
Commande d'audit d'architecture : npm run arch:report

CE QUE JE TE DONNE
- Le test à faire passer : tests/tests-unitaires/facturation/CASE-FAC-701.test.ts::test_CASE_FAC_701_emission_facture_acompte_puis_solde_privatisation_forfaitaire
- Le cas de test : tests/cases/facturation/CASE-FAC-701.md
- La spécification : specs/facturation.md, section SPEC-FAC-02 (AC-1, AC-2, AC-3, AC-6, Scénario 2)
...
TA TÂCHE
1. Fais passer au vert le test test_CASE_FAC_701_emission_facture_acompte_puis_solde_privatisation_forfaitaire, et lui seul.
...
4. La règle vit dans le domaine : src/utils/calcul-acompte-solde.util.ts (taux d'acompte 50 % applicable aux formules de privatisation) et src/utils/calcul-tarification-facturation.util.ts (montant forfaitaire fixe, sans ventilation par passager).
