...
Commande pour ce seul test : npx vitest run tests/tests-unitaires/facturation/CASE-FAC-706.test.ts -t "test_CASE_FAC_706_facturation_acompte_solde_privatisation_saint_leu_sans_majoration"
Commande d'audit d'architecture : npm run arch:report

CE QUE JE TE DONNE
- Le test à faire passer : tests/tests-unitaires/facturation/CASE-FAC-706.test.ts::test_CASE_FAC_706_facturation_acompte_solde_privatisation_saint_leu_sans_majoration
- Le cas de test : tests/cases/facturation/CASE-FAC-706.md
- La spécification : specs/facturation.md, section SPEC-FAC-02 (AC-1, AC-2, AC-3, Scénario 2, Cas limite #5)
- Les exigences d'architecture : specs/architecture.md (SPEC-ARCH-01, SPEC-ARCH-02, SPEC-ARCH-03)
- Les couches du domaine que tu peux modifier : src/utils/, src/services/server/

TA TÂCHE
1. Fais passer au vert le test test_CASE_FAC_706_facturation_acompte_solde_privatisation_saint_leu_sans_majoration, et lui seul.
...
4. La règle vit dans le domaine : src/utils/calcul-tarification-facturation.util.ts (forfait de privatisation fixe non soumis à la majoration géographique par passager).
...
7. Ne modifie aucun fichier en dehors de : src/utils/, src/services/server/
