...
Commande pour ce seul test : npx vitest run tests/tests-unitaires/facturation/CASE-FAC-721.test.ts -t "test_CASE_FAC_721_non_declenchement_facturation_abandon_ou_expiration_session_paiement_acompte_ou_solde"
Commande d'audit d'architecture : npm run arch:report

CE QUE JE TE DONNE
- Le test à faire passer : tests/tests-unitaires/facturation/CASE-FAC-721.test.ts::test_CASE_FAC_721_non_declenchement_facturation_abandon_ou_expiration_session_paiement_acompte_ou_solde
- Le cas de test : tests/cases/facturation/CASE-FAC-721.md
- La spécification : specs/facturation.md, section SPEC-FAC-02 (AC-7, Cas limite #3)
- Les exigences d'architecture : specs/architecture.md (SPEC-ARCH-01, SPEC-ARCH-02, SPEC-ARCH-03)
- Les couches du domaine que tu peux modifier : src/actions/

TA TÂCHE
1. Fais passer au vert le test test_CASE_FAC_721_non_declenchement_facturation_abandon_ou_expiration_session_paiement_acompte_ou_solde, et lui seul.
...
4. La règle vit dans le domaine : src/actions/emettre-facture-acompte-apres-paiement.ts et src/actions/emettre-facture-solde-apres-paiement.ts (filtrage des statuts de paiement non validés).
...
7. Ne modifie aucun fichier en dehors de : src/actions/
