...
Commande pour ce seul test : npx vitest run tests/tests-unitaires/facturation/CASE-FAC-705.test.ts -t "test_CASE_FAC_705_majoration_saint_leu_sur_profil_enfant_facture_acompte"
Commande d'audit d'architecture : npm run arch:report

CE QUE JE TE DONNE
- Le test à faire passer : tests/tests-unitaires/facturation/CASE-FAC-705.test.ts::test_CASE_FAC_705_majoration_saint_leu_sur_profil_enfant_facture_acompte
- Le cas de test : tests/cases/facturation/CASE-FAC-705.md
...
TA TÂCHE
1. Fais passer au vert le test test_CASE_FAC_705_majoration_saint_leu_sur_profil_enfant_facture_acompte, et lui seul.
...
4. La règle vit dans le domaine : src/utils/calcul-tarification-facturation.util.ts (majoration géographique appliquée à chaque passager individuel, enfants compris).
