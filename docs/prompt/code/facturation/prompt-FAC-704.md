...
Commande pour ce seul test : npx vitest run tests/tests-unitaires/facturation/CASE-FAC-704.test.ts -t "test_CASE_FAC_704_facturation_acompte_mixte_adultes_enfants_ventilation_detaillee_pdf"
Commande d'audit d'architecture : npm run arch:report

CE QUE JE TE DONNE
- Le test à faire passer : tests/tests-unitaires/facturation/CASE-FAC-704.test.ts::test_CASE_FAC_704_facturation_acompte_mixte_adultes_enfants_ventilation_detaillee_pdf
- Le cas de test : tests/cases/facturation/CASE-FAC-704.md
- La spécification : specs/facturation.md, section SPEC-FAC-02 (AC-1, AC-3)
...
TA TÂCHE
1. Fais passer au vert le test test_CASE_FAC_704_facturation_acompte_mixte_adultes_enfants_ventilation_detaillee_pdf, et lui seul.
...
4. La règle vit dans le domaine : src/utils/calcul-tarification-facturation.util.ts (ventilation par catégorie adultes/enfants) et src/services/server/generer-facture-acompte-pdf.service.ts (lignes de détail distinctes).
