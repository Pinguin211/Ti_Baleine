...
Commande pour ce seul test : npx vitest run tests/tests-unitaires/facturation/CASE-FAC-703.test.ts -t "test_CASE_FAC_703_facturation_acompte_sortie_dauphins_tarif_specifique"
Commande d'audit d'architecture : npm run arch:report

CE QUE JE TE DONNE
- Le test à faire passer : tests/tests-unitaires/facturation/CASE-FAC-703.test.ts::test_CASE_FAC_703_facturation_acompte_sortie_dauphins_tarif_specifique
- Le cas de test : tests/cases/facturation/CASE-FAC-703.md
...
TA TÂCHE
1. Fais passer au vert le test test_CASE_FAC_703_facturation_acompte_sortie_dauphins_tarif_specifique, et lui seul.
...
4. La règle vit dans le domaine : src/utils/calcul-tarification-facturation.util.ts (grille tarifaire propre à l'activité Dauphins).
