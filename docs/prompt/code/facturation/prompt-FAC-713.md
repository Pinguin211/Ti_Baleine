...
Commande pour ce seul test : npx vitest run tests/tests-unitaires/facturation/CASE-FAC-713.test.ts -t "test_CASE_FAC_713_generation_facture_pdf_exclusivement_en_memoire_sans_persistance_disque"
Commande d'audit d'architecture : npm run arch:report

CE QUE JE TE DONNE
- Le test à faire passer : tests/tests-unitaires/facturation/CASE-FAC-713.test.ts::test_CASE_FAC_713_generation_facture_pdf_exclusivement_en_memoire_sans_persistance_disque
- Le cas de test : tests/cases/facturation/CASE-FAC-713.md
- La spécification : specs/facturation.md, section SPEC-FAC-02 (AC-1, AC-2, Règle, Portée §5)
...
TA TÂCHE
1. Fais passer au vert le test test_CASE_FAC_713_generation_facture_pdf_exclusivement_en_memoire_sans_persistance_disque, et lui seul.
...
4. La règle vit dans le domaine : src/services/server/generer-facture-acompte-pdf.service.ts et src/services/server/generer-facture-solde-pdf.service.ts (génération exclusivement en mémoire, sans écriture disque).
