# Prompt — écrire le code de CASE-ADMIN-030

> Rejet strict de toute tentative de réduction de passagers sur un créneau déjà passé
>
> Étape « test → code ». Le test est le contrat, l'agent produit le code. Suit
> `docs/prompt/prompt-gabarit-code.md`.
>
> Prérequis : le test `tests/tests-unitaires/admin/case-admin-030.test.ts` doit exister et être rouge.
>
...
Commande pour ce seul test : npx vitest run --project admin tests/tests-unitaires/admin/case-admin-030.test.ts
Commande d'audit d'architecture : npm run arch:report

CE QUE JE TE DONNE
- Le test à faire passer : tests/tests-unitaires/admin/case-admin-030.test.ts::test_CASE_ADMIN_030_rejet_strict_reduction_passagers_creneau_deja_passe
...
TA TÂCHE
1. Fais passer au vert le test test_CASE_ADMIN_030_rejet_strict_reduction_passagers_creneau_deja_passe, et lui seul.
