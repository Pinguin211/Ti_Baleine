# Prompt — écrire le code de CASE-ADMIN-050

> Envoi d'une alerte de pré-annulation via le canal E-mail uniquement
>
> Étape « test → code ». Le test est le contrat, l'agent produit le code. Suit
> `docs/prompt/prompt-gabarit-code.md`.
>
> Prérequis : le test `tests/tests-unitaires/admin/case-admin-050.test.ts` doit exister et être rouge.
>
...
Commande pour ce seul test : npx vitest run --project admin tests/tests-unitaires/admin/case-admin-050.test.ts
Commande d'audit d'architecture : npm run arch:report

CE QUE JE TE DONNE
- Le test à faire passer : tests/tests-unitaires/admin/case-admin-050.test.ts::test_CASE_ADMIN_050_envoi_alerte_pre_annulation_canal_email_uniquement
...
TA TÂCHE
1. Fais passer au vert le test test_CASE_ADMIN_050_envoi_alerte_pre_annulation_canal_email_uniquement, et lui seul.
