# Prompt — écrire le code de CASE-ADMIN-024

> Réduction partielle de passagers par suppression sélective de N billets enfants
>
> Étape « test → code ». Le test est le contrat, l'agent produit le code. Suit
> `docs/prompt/prompt-gabarit-code.md`.
>
> Prérequis : le test `tests/tests-unitaires/admin/case-admin-024.test.ts` doit exister et être rouge.
>
...
Commande pour ce seul test : npx vitest run --project admin tests/tests-unitaires/admin/case-admin-024.test.ts
Commande d'audit d'architecture : npm run arch:report

CE QUE JE TE DONNE
- Le test à faire passer : tests/tests-unitaires/admin/case-admin-024.test.ts::test_CASE_ADMIN_024_reduction_partielle_passagers_suppression_billet_enfant
...
TA TÂCHE
1. Fais passer au vert le test test_CASE_ADMIN_024_reduction_partielle_passagers_suppression_billet_enfant, et lui seul.
