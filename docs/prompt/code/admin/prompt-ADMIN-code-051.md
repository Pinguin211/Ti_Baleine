# Prompt — écrire le code de CASE-ADMIN-051

> Envoi combiné simultané d'une alerte de pré-annulation par SMS et E-mail
>
> Étape « test → code ». Le test est le contrat, l'agent produit le code. Suit
> `docs/prompt/prompt-gabarit-code.md`.
>
> Prérequis : le test `tests/tests-unitaires/admin/case-admin-051.test.ts` doit exister et être rouge.
>
...
Commande pour ce seul test : npx vitest run --project admin tests/tests-unitaires/admin/case-admin-051.test.ts
Commande d'audit d'architecture : npm run arch:report

CE QUE JE TE DONNE
- Le test à faire passer : tests/tests-unitaires/admin/case-admin-051.test.ts::test_CASE_ADMIN_051_envoi_combine_simultane_alerte_sms_email
...
TA TÂCHE
1. Fais passer au vert le test test_CASE_ADMIN_051_envoi_combine_simultane_alerte_sms_email, et lui seul.
