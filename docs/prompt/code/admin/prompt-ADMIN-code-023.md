# Prompt — écrire le code de CASE-ADMIN-023

> Réduction partielle de passagers par suppression sélective de N billets adultes
>
> Étape « test → code ». Le test est le contrat, l'agent produit le code. Suit
> `docs/prompt/prompt-gabarit-code.md`.
>
> Prérequis : le test `tests/tests-unitaires/admin/case-admin-023.test.ts` doit exister et être rouge.
>
...
Commande pour ce seul test : npx vitest run --project admin tests/tests-unitaires/admin/case-admin-023.test.ts
Commande d'audit d'architecture : npm run arch:report

CE QUE JE TE DONNE
- Le test à faire passer : tests/tests-unitaires/admin/case-admin-023.test.ts::test_CASE_ADMIN_023_reduction_partielle_passagers_suppression_billet_adulte
...
TA TÂCHE
1. Fais passer au vert le test test_CASE_ADMIN_023_reduction_partielle_passagers_suppression_billet_adulte, et lui seul.
