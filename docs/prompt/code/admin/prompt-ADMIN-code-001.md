# Prompt — écrire le code de CASE-ADMIN-001

> Affichage consolidé de la grille multi-sites sur poste Desktop
>
> Étape « test → code ». Le test est le contrat, l'agent produit le code. Suit
> `docs/prompt/prompt-gabarit-code.md`.
>
> Prérequis : le test `tests/tests-unitaires/admin/case-admin-001.test.ts` doit exister et être rouge.
>
...
Commande pour ce seul test : npx vitest run --project admin tests/tests-unitaires/admin/case-admin-001.test.ts
Commande d'audit d'architecture : npm run arch:report

CE QUE JE TE DONNE
- Le test à faire passer : tests/tests-unitaires/admin/case-admin-001.test.ts::test_CASE_ADMIN_001_affichage_consolide_grille_planning_multisites_desktop
...
TA TÂCHE
1. Fais passer au vert le test test_CASE_ADMIN_001_affichage_consolide_grille_planning_multisites_desktop, et lui seul.
