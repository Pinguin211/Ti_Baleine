# Prompt — écrire le code de CASE-ADMIN-055

> Présence obligatoire du message bilingue combiné FR + EN dans le corps unique de message
>
> Étape « test → code ». Le test est le contrat, l'agent produit le code. Suit
> `docs/prompt/prompt-gabarit-code.md`.
>
> Prérequis : le test `tests/tests-unitaires/admin/case-admin-055.test.ts` doit exister et être rouge.
>
...
Commande pour ce seul test : npx vitest run --project admin tests/tests-unitaires/admin/case-admin-055.test.ts
Commande d'audit d'architecture : npm run arch:report

CE QUE JE TE DONNE
- Le test à faire passer : tests/tests-unitaires/admin/case-admin-055.test.ts::test_CASE_ADMIN_055_presence_obligatoire_message_bilingue_combine_fr_en
...
TA TÂCHE
1. Fais passer au vert le test test_CASE_ADMIN_055_presence_obligatoire_message_bilingue_combine_fr_en, et lui seul.
