# Prompt — écrire le code de CASE-ADMIN-027

> Rejet strict de toute tentative d'ajout de billet sur une réservation existante
>
> Étape « test → code ». Le test est le contrat, l'agent produit le code. Suit
> `docs/prompt/prompt-gabarit-code.md`.
>
> Prérequis : le test `tests/tests-unitaires/admin/case-admin-027.test.ts` doit exister et être rouge.
>
...
Commande pour ce seul test : npx vitest run --project admin tests/tests-unitaires/admin/case-admin-027.test.ts
Commande d'audit d'architecture : npm run arch:report

CE QUE JE TE DONNE
- Le test à faire passer : tests/tests-unitaires/admin/case-admin-027.test.ts::test_CASE_ADMIN_027_rejet_strict_tentative_ajout_billet_reservation_existante
...
TA TÂCHE
1. Fais passer au vert le test test_CASE_ADMIN_027_rejet_strict_tentative_ajout_billet_reservation_existante, et lui seul.
