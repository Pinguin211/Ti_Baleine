# Prompt — écrire le code de CASE-ADMIN-025

> Réduction partielle mixte (adultes et enfants) avec recalcul immédiat et trace d'audit
>
> Étape « test → code ». Le test est le contrat, l'agent produit le code. Suit
> `docs/prompt/prompt-gabarit-code.md`.
>
> Prérequis : le test `tests/tests-unitaires/admin/case-admin-025.test.ts` doit exister et être rouge.
>
...
Commande pour ce seul test : npx vitest run --project admin tests/tests-unitaires/admin/case-admin-025.test.ts
Commande d'audit d'architecture : npm run arch:report

CE QUE JE TE DONNE
- Le test à faire passer : tests/tests-unitaires/admin/case-admin-025.test.ts::test_CASE_ADMIN_025_reduction_mixte_adultes_enfants_recalcul_audit
...
TA TÂCHE
1. Fais passer au vert le test test_CASE_ADMIN_025_reduction_mixte_adultes_enfants_recalcul_audit, et lui seul.
