# Prompt — écrire le code de CASE-ADMIN-071

> Maintien de l'état authentifié lors de la navigation inter-pages et du rafraîchissement (F5)
>
> Étape « test → code ». Le test est le contrat, l'agent produit le code. Suit
> `docs/prompt/prompt-gabarit-code.md`.
>
> Prérequis : le test `tests/tests-unitaires/admin/case-admin-071.test.ts` doit exister et être rouge.
>
...
Commande pour ce seul test : npx vitest run --project admin tests/tests-unitaires/admin/case-admin-071.test.ts
Commande d'audit d'architecture : npm run arch:report

CE QUE JE TE DONNE
- Le test à faire passer : tests/tests-unitaires/admin/case-admin-071.test.ts::test_CASE_ADMIN_071_maintien_etat_authentifie_navigation_rafraichissement_f5
...
TA TÂCHE
1. Fais passer au vert le test test_CASE_ADMIN_071_maintien_etat_authentifie_navigation_rafraichissement_f5, et lui seul.
