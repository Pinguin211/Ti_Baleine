# Prompt — écrire le code de CASE-ADMIN-007

> Consultation du planning en continu 24h/24 sans restriction horaire
>
> Étape « test → code ». Le test est le contrat, l'agent produit le code. Suit
> `docs/prompt/prompt-gabarit-code.md`.
>
> Prérequis : le test `tests/tests-unitaires/admin/case-admin-007.test.ts` doit exister et être rouge.
>
...
Commande pour ce seul test : npx vitest run --project admin tests/tests-unitaires/admin/case-admin-007.test.ts
Commande d'audit d'architecture : npm run arch:report

CE QUE JE TE DONNE
- Le test à faire passer : tests/tests-unitaires/admin/case-admin-007.test.ts::test_CASE_ADMIN_007_consultation_planning_continu_24h_24_sans_restriction
...
TA TÂCHE
1. Fais passer au vert le test test_CASE_ADMIN_007_consultation_planning_continu_24h_24_sans_restriction, et lui seul.
