# Prompt — écrire le code de CASE-ADMIN-076

> Rejet strict de tout encaissement en espèces ou en chèques vacances
>
> Étape « test → code ». Le test est le contrat, l'agent produit le code. Suit
> `docs/prompt/prompt-gabarit-code.md`.
>
> Prérequis : le test `tests/tests-unitaires/admin/case-admin-076.test.ts` doit exister et être rouge.
>
...
Commande pour ce seul test : npx vitest run --project admin tests/tests-unitaires/admin/case-admin-076.test.ts
Commande d'audit d'architecture : npm run arch:report

CE QUE JE TE DONNE
- Le test à faire passer : tests/tests-unitaires/admin/case-admin-076.test.ts::test_CASE_ADMIN_076_rejet_encaissement_especes_cheques_vacances_absent_interface
- Le cas de test : tests/cases/admin/CASE-ADMIN-076.md
...
TA TÂCHE
1. Fais passer au vert le test test_CASE_ADMIN_076_rejet_encaissement_especes_cheques_vacances_absent_interface, et lui seul.
