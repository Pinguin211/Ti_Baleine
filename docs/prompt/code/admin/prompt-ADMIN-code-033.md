# Prompt — écrire le code de CASE-ADMIN-033

> Connexion réussie de l'administrateur avec un identifiant e-mail et un mot de passe valides sur Desktop
>
> Étape « test → code ». Le test est le contrat, l'agent produit le code. Suit
> `docs/prompt/prompt-gabarit-code.md`.
>
> Prérequis : le test `tests/tests-unitaires/admin/case-admin-033.test.ts` doit exister et être rouge.
>
...
Commande pour ce seul test : npx vitest run --project admin tests/tests-unitaires/admin/case-admin-033.test.ts
Commande d'audit d'architecture : npm run arch:report

CE QUE JE TE DONNE
- Le test à faire passer : tests/tests-unitaires/admin/case-admin-033.test.ts::test_CASE_ADMIN_033_connexion_reussie_administrateur_identifiants_valides_desktop
...
- Les fichiers du domaine que tu peux modifier : src/actions/, src/app/admin/, src/lib/server/auth/, src/schemas/validation/auth/

TA TÂCHE
1. Fais passer au vert le test test_CASE_ADMIN_033_connexion_reussie_administrateur_identifiants_valides_desktop, et lui seul.
...
4. La règle vit dans le domaine : src/lib/server/auth/. Pas dans un contrôleur, pas dans un
...
8. Ne modifie aucun fichier en dehors de : `src/actions/`, `src/app/admin/`, `src/lib/server/auth/`, `src/schemas/validation/auth/`.
