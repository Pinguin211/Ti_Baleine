# Prompt — écrire le code de CASE-ADMIN-061

> Traitement d'un échec individuel de délivrance lors d'une alerte groupée : journalisation sans blocage de la file
>
> Étape « test → code ». Le test est le contrat, l'agent produit le code. Suit
> `docs/prompt/prompt-gabarit-code.md`.
>
> Prérequis : le test `tests/tests-unitaires/admin/case-admin-061.test.ts` doit exister et être rouge.
>
...
Commande pour ce seul test : npx vitest run --project admin tests/tests-unitaires/admin/case-admin-061.test.ts
Commande d'audit d'architecture : npm run arch:report

CE QUE JE TE DONNE
- Le test à faire passer : tests/tests-unitaires/admin/case-admin-061.test.ts::test_CASE_ADMIN_061_traitement_echec_individuel_delivrance_alerte_groupee_log
...
TA TÂCHE
1. Fais passer au vert le test test_CASE_ADMIN_061_traitement_echec_individuel_delivrance_alerte_groupee_log, et lui seul.
