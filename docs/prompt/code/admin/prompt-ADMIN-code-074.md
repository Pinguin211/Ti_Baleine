# Prompt — écrire le code de CASE-ADMIN-074

> Encaissement du solde par carte bancaire sur place et bascule vers « Payée complètement »
>
> Étape « test → code ». Le test est le contrat, l'agent produit le code. Suit
> `docs/prompt/prompt-gabarit-code.md`.
>
> Prérequis : le test `tests/tests-unitaires/admin/case-admin-074.test.ts` doit exister et être rouge.
>
...
Commande pour ce seul test : npx vitest run --project admin tests/tests-unitaires/admin/case-admin-074.test.ts
Commande d'audit d'architecture : npm run arch:report

CE QUE JE TE DONNE
- Le test à faire passer : tests/tests-unitaires/admin/case-admin-074.test.ts::test_CASE_ADMIN_074_encaissement_solde_cb_sur_place_bascule_payee_completement
- Le cas de test : tests/cases/admin/CASE-ADMIN-074.md
- La spécification : specs/admin.md, section SPEC-ADMIN-08
- Les exigences d'architecture : specs/architecture.md
- Le modèle du domaine : docs/uml/domain.puml
- Les fichiers du domaine que tu peux modifier : src/actions/, src/services/server/payment/, src/schemas/validation/payment/
- Code existant réutilisable en lecture seule, à ne PAS modifier : src/actions/emettre-facture-apres-paiement.ts, src/services/server/generer-facture-pdf.ts (SPEC-FAC-02, facture de solde distincte)

TA TÂCHE
1. Fais passer au vert le test test_CASE_ADMIN_074_encaissement_solde_cb_sur_place_bascule_payee_completement, et lui seul.
...
4. La règle vit dans le domaine : src/services/server/payment/. Pas dans un contrôleur, pas dans un
...
8. Ne modifie aucun fichier en dehors de : `src/actions/`, `src/services/server/payment/`, `src/schemas/validation/payment/`.
