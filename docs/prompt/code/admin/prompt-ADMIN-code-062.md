# Prompt — écrire le code de CASE-ADMIN-062

> Fermeture administrative manuelle d'un créneau ouvert sans passager
>
> Étape « test → code ». Le test est le contrat, l'agent produit le code. Suit
> `docs/prompt/prompt-gabarit-code.md`.
>
> Prérequis : le test `tests/tests-unitaires/admin/case-admin-062.test.ts` doit exister et être rouge.
>
...
Commande pour ce seul test : npx vitest run --project admin tests/tests-unitaires/admin/case-admin-062.test.ts
Commande d'audit d'architecture : npm run arch:report

CE QUE JE TE DONNE
- Le test à faire passer : tests/tests-unitaires/admin/case-admin-062.test.ts::test_CASE_ADMIN_062_fermeture_administrative_manuelle_creneau_sans_passager
...
TA TÂCHE
1. Fais passer au vert le test test_CASE_ADMIN_062_fermeture_administrative_manuelle_creneau_sans_passager, et lui seul.
