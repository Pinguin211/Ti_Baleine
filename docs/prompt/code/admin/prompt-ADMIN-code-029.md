# Prompt — écrire le code de CASE-ADMIN-029

> Blocage de l'action de réduction sur une réservation n'ayant déjà plus aucun billet actif
>
> Étape « test → code ». Le test est le contrat, l'agent produit le code. Suit
> `docs/prompt/prompt-gabarit-code.md`.
>
> Prérequis : le test `tests/tests-unitaires/admin/case-admin-029.test.ts` doit exister et être rouge.
>
...
Commande pour ce seul test : npx vitest run --project admin tests/tests-unitaires/admin/case-admin-029.test.ts
Commande d'audit d'architecture : npm run arch:report

CE QUE JE TE DONNE
- Le test à faire passer : tests/tests-unitaires/admin/case-admin-029.test.ts::test_CASE_ADMIN_029_blocage_action_reduction_reservation_0_billet_actif
...
TA TÂCHE
1. Fais passer au vert le test test_CASE_ADMIN_029_blocage_action_reduction_reservation_0_billet_actif, et lui seul.
