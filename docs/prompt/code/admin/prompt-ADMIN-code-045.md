# Prompt — écrire le code de CASE-ADMIN-045

> Affichage d'un créneau complet avec affichage à 100 % et badge « Complet »
>
> Étape « test → code ». Le test est le contrat, l'agent produit le code. Suit
> `docs/prompt/prompt-gabarit-code.md`.
>
> Prérequis : le test `tests/tests-unitaires/admin/case-admin-045.test.ts` doit exister et être rouge.
>
...
Commande pour ce seul test : npx vitest run --project admin tests/tests-unitaires/admin/case-admin-045.test.ts
Commande d'audit d'architecture : npm run arch:report

CE QUE JE TE DONNE
- Le test à faire passer : tests/tests-unitaires/admin/case-admin-045.test.ts::test_CASE_ADMIN_045_affichage_creneau_complet_taux_100_pourcent_badge_complet
...
TA TÂCHE
1. Fais passer au vert le test test_CASE_ADMIN_045_affichage_creneau_complet_taux_100_pourcent_badge_complet, et lui seul.
