# Prompt — écrire le code de CASE-ADMIN-046

> Recalcul instantané en temps réel du nombre de billets actifs et du taux de remplissage lors d'une annulation ou réduction
>
> Étape « test → code ». Le test est le contrat, l'agent produit le code. Suit
> `docs/prompt/prompt-gabarit-code.md`.
>
> Prérequis : le test `tests/tests-unitaires/admin/case-admin-046.test.ts` doit exister et être rouge.
>
...
Commande pour ce seul test : npx vitest run --project admin tests/tests-unitaires/admin/case-admin-046.test.ts
Commande d'audit d'architecture : npm run arch:report

CE QUE JE TE DONNE
- Le test à faire passer : tests/tests-unitaires/admin/case-admin-046.test.ts::test_CASE_ADMIN_046_recalcul_instantane_temps_reel_remplissage_apres_annulation
...
TA TÂCHE
1. Fais passer au vert le test test_CASE_ADMIN_046_recalcul_instantane_temps_reel_remplissage_apres_annulation, et lui seul.
