# Prompt — écrire le code de CASE-ADMIN-036

> Saisie d'un identifiant ou mot de passe invalide : refus d'accès et message d'erreur générique
>
> Étape « test → code ». Le test est le contrat, l'agent produit le code. Suit
> `docs/prompt/prompt-gabarit-code.md`.
>
> Prérequis : le test `tests/tests-unitaires/admin/case-admin-036.test.ts` doit exister et être rouge.
>
...
Commande pour ce seul test : npx vitest run --project admin tests/tests-unitaires/admin/case-admin-036.test.ts
Commande d'audit d'architecture : npm run arch:report

CE QUE JE TE DONNE
- Le test à faire passer : tests/tests-unitaires/admin/case-admin-036.test.ts::test_CASE_ADMIN_036_refus_connexion_identifiant_invalide_message_generique
...
TA TÂCHE
1. Fais passer au vert le test test_CASE_ADMIN_036_refus_connexion_identifiant_invalide_message_generique, et lui seul.
