# Prompt — écrire le code de CASE-ADMIN-058

> Désactivation du bouton de déclenchement d'envoi lorsqu'aucun créneau du lendemain n'est sélectionné
>
> Étape « test → code ». Le test est le contrat, l'agent produit le code. Suit
> `docs/prompt/prompt-gabarit-code.md`.
>
> Prérequis : le test `tests/tests-unitaires/admin/case-admin-058.test.ts` doit exister et être rouge.
>
...
Commande pour ce seul test : npx vitest run --project admin tests/tests-unitaires/admin/case-admin-058.test.ts
Commande d'audit d'architecture : npm run arch:report

CE QUE JE TE DONNE
- Le test à faire passer : tests/tests-unitaires/admin/case-admin-058.test.ts::test_CASE_ADMIN_058_desactivation_bouton_envoi_aucun_creneau_selectionne
...
TA TÂCHE
1. Fais passer au vert le test test_CASE_ADMIN_058_desactivation_bouton_envoi_aucun_creneau_selectionne, et lui seul.
