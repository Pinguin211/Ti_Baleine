# Prompt — écrire le code de CASE-ADMIN-064

> Configuration et modification de l'affectation du type d'activité sur un créneau
>
> Étape « test → code ». Le test est le contrat, l'agent produit le code. Suit
> `docs/prompt/prompt-gabarit-code.md`.
>
> Prérequis : le test `tests/tests-unitaires/admin/case-admin-064.test.ts` doit exister et être rouge.
>
...
Commande pour ce seul test : npx vitest run --project admin tests/tests-unitaires/admin/case-admin-064.test.ts
Commande d'audit d'architecture : npm run arch:report

CE QUE JE TE DONNE
- Le test à faire passer : tests/tests-unitaires/admin/case-admin-064.test.ts::test_CASE_ADMIN_064_configuration_modification_affectation_activite_creneau
...
TA TÂCHE
1. Fais passer au vert le test test_CASE_ADMIN_064_configuration_modification_affectation_activite_creneau, et lui seul.
