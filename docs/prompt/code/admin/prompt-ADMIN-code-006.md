# Prompt — écrire le code de CASE-ADMIN-006

> Affichage distinctif d'un créneau sans type d'activité avec le statut « type non renseigné »
>
> Étape « test → code ». Le test est le contrat, l'agent produit le code. Suit
> `docs/prompt/prompt-gabarit-code.md`.
>
> Prérequis : le test `tests/tests-unitaires/admin/case-admin-006.test.ts` doit exister et être rouge.
>
...
Commande pour ce seul test : npx vitest run --project admin tests/tests-unitaires/admin/case-admin-006.test.ts
Commande d'audit d'architecture : npm run arch:report

CE QUE JE TE DONNE
- Le test à faire passer : tests/tests-unitaires/admin/case-admin-006.test.ts::test_CASE_ADMIN_006_affichage_creneau_sans_activite_type_non_renseigne
...
TA TÂCHE
1. Fais passer au vert le test test_CASE_ADMIN_006_affichage_creneau_sans_activite_type_non_renseigne, et lui seul.
