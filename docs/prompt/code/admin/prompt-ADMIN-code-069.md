# Prompt — écrire le code de CASE-ADMIN-069

> Rejet et blocage à la saisie lors d'une tentative de suppression d'un nombre de billets supérieur aux billets actifs
>
> Étape « test → code ». Le test est le contrat, l'agent produit le code. Suit
> `docs/prompt/prompt-gabarit-code.md`.
>
> Prérequis : le test `tests/tests-unitaires/admin/case-admin-069.test.ts` doit exister et être rouge.
>
...
Commande pour ce seul test : npx vitest run --project admin tests/tests-unitaires/admin/case-admin-069.test.ts
Commande d'audit d'architecture : npm run arch:report

CE QUE JE TE DONNE
- Le test à faire passer : tests/tests-unitaires/admin/case-admin-069.test.ts::test_CASE_ADMIN_069_rejet_suppression_nombre_billets_superieur_solde_actif
...
TA TÂCHE
1. Fais passer au vert le test test_CASE_ADMIN_069_rejet_suppression_nombre_billets_superieur_solde_actif, et lui seul.
