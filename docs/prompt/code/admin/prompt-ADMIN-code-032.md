# Prompt — écrire le code de CASE-ADMIN-032

> Absence de remboursement financier automatique consécutif à une réduction partielle de passagers
>
> Étape « test → code ». Le test est le contrat, l'agent produit le code. Suit
> `docs/prompt/prompt-gabarit-code.md`.
>
> Prérequis : le test `tests/tests-unitaires/admin/case-admin-032.test.ts` doit exister et être rouge.
>
...
Commande pour ce seul test : npx vitest run --project admin tests/tests-unitaires/admin/case-admin-032.test.ts
Commande d'audit d'architecture : npm run arch:report

CE QUE JE TE DONNE
- Le test à faire passer : tests/tests-unitaires/admin/case-admin-032.test.ts::test_CASE_ADMIN_032_absence_remboursement_financier_automatique_reduction_partielle
