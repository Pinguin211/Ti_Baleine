# Prompt — écrire le code de CASE-ADMIN-037

> Blocage à la validation du formulaire de connexion lorsque des champs obligatoires sont laissés vides
>
> Étape « test → code ». Le test est le contrat, l'agent produit le code. Suit
> `docs/prompt/prompt-gabarit-code.md`.
>
> Prérequis : le test `tests/tests-unitaires/admin/case-admin-037.test.ts` doit exister et être rouge.
>
...
Commande pour ce seul test : npx vitest run --project admin tests/tests-unitaires/admin/case-admin-037.test.ts
Commande d'audit d'architecture : npm run arch:report

CE QUE JE TE DONNE
- Le test à faire passer : tests/tests-unitaires/admin/case-admin-037.test.ts::test_CASE_ADMIN_037_blocage_validation_formulaire_connexion_champs_vides
...
TA TÂCHE
1. Fais passer au vert le test test_CASE_ADMIN_037_blocage_validation_formulaire_connexion_champs_vides, et lui seul.
