# Prompt — écrire le code de CASE-ADMIN-002

> Consultation du détail d'un créneau avec activité et navires mobilisés
>
> Étape « test → code ». Le test est le contrat, l'agent produit le code. Suit
> `docs/prompt/prompt-gabarit-code.md`.
>
> Prérequis : le test `tests/tests-unitaires/admin/case-admin-002.test.ts` doit exister et être rouge.
>
...
Commande pour ce seul test : npx vitest run --project admin tests/tests-unitaires/admin/case-admin-002.test.ts
Commande d'audit d'architecture : npm run arch:report

CE QUE JE TE DONNE
- Le test à faire passer : tests/tests-unitaires/admin/case-admin-002.test.ts::test_CASE_ADMIN_002_consultation_detail_creneau_activite_navires_mobilises
...
TA TÂCHE
1. Fais passer au vert le test test_CASE_ADMIN_002_consultation_detail_creneau_activite_navires_mobilises, et lui seul.
