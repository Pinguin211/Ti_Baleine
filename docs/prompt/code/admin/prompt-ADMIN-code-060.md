# Prompt — écrire le code de CASE-ADMIN-060
...
>
> Prérequis : le test `tests/tests-unitaires/admin/case-admin-060.test.ts` doit exister et être rouge.
>
...
Commande pour ce seul test : npx vitest run --project admin tests/tests-unitaires/admin/case-admin-060.test.ts
Commande d'audit d'architecture : npm run arch:report

CE QUE JE TE DONNE
- Le test à faire passer : tests/tests-unitaires/admin/case-admin-060.test.ts::test_CASE_ADMIN_060_blocage_envoi_alerte_corps_message_vide
...
TA TÂCHE
1. Fais passer au vert le test test_CASE_ADMIN_060_blocage_envoi_alerte_corps_message_vide, et lui seul.
