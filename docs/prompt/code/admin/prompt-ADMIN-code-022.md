# Prompt — écrire le code de CASE-ADMIN-022

> Absence de flux financier sortant automatisé lors de l'annulation
>
> Étape « test → code ». Le test est le contrat, l'agent produit le code. Suit
> `docs/prompt/prompt-gabarit-code.md`.
>
> Prérequis : le test `tests/tests-unitaires/admin/case-admin-022.test.ts` doit exister et être rouge.
>
...
Commande pour ce seul test : npx vitest run --project admin tests/tests-unitaires/admin/case-admin-022.test.ts
Commande d'audit d'architecture : npm run arch:report

CE QUE JE TE DONNE
- Le test à faire passer : tests/tests-unitaires/admin/case-admin-022.test.ts::test_CASE_ADMIN_022_absence_flux_financier_sortant_automatique_annulation
...
TA TÂCHE
1. Fais passer au vert le test test_CASE_ADMIN_022_absence_flux_financier_sortant_automatique_annulation, et lui seul.
