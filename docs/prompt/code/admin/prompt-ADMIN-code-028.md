# Prompt — écrire le code de CASE-ADMIN-028

> Rejet strict de toute tentative de modification de la date ou du port lors d'une réduction
>
> Étape « test → code ». Le test est le contrat, l'agent produit le code. Suit
> `docs/prompt/prompt-gabarit-code.md`.
>
> Prérequis : le test `tests/tests-unitaires/admin/case-admin-028.test.ts` doit exister et être rouge.
>
...
Commande pour ce seul test : npx vitest run --project admin tests/tests-unitaires/admin/case-admin-028.test.ts
Commande d'audit d'architecture : npm run arch:report

CE QUE JE TE DONNE
- Le test à faire passer : tests/tests-unitaires/admin/case-admin-028.test.ts::test_CASE_ADMIN_028_rejet_strict_modification_date_port_lors_reduction
...
TA TÂCHE
1. Fais passer au vert le test test_CASE_ADMIN_028_rejet_strict_modification_date_port_lors_reduction, et lui seul.
