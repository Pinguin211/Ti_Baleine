# Prompt — écrire le code de CASE-ADMIN-057

> Activation immédiate de la mention d'avertissement sur le site public pour les créneaux sous pré-alerte ayant des places ouvertes
>
> Étape « test → code ». Le test est le contrat, l'agent produit le code. Suit
> `docs/prompt/prompt-gabarit-code.md`.
>
> Prérequis : le test `tests/tests-unitaires/admin/case-admin-057.test.ts` doit exister et être rouge.
>
...
Commande pour ce seul test : npx vitest run --project admin tests/tests-unitaires/admin/case-admin-057.test.ts
Commande d'audit d'architecture : npm run arch:report

CE QUE JE TE DONNE
- Le test à faire passer : tests/tests-unitaires/admin/case-admin-057.test.ts::test_CASE_ADMIN_057_activation_mention_avertissement_site_public_creneau_pre_alerte
...
- Les fichiers du domaine que tu peux modifier : src/actions/, src/services/server/alerts/, src/schemas/validation/alerts/, src/lib/sms/, src/lib/email/

TA TÂCHE
1. Fais passer au vert le test test_CASE_ADMIN_057_activation_mention_avertissement_site_public_creneau_pre_alerte, et lui seul.
...
4. La règle vit dans le domaine : src/services/server/alerts/. Pas dans un contrôleur, pas dans un
...
8. Ne modifie aucun fichier en dehors de : `src/actions/`, `src/services/server/alerts/`, `src/schemas/validation/alerts/`, `src/lib/sms/`, `src/lib/email/`.
