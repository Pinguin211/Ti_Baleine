# Prompt — écrire le code de CASE-ADMIN-053

> Préremplissage instantané de la zone de texte par sélection du template type codé en dur « Incident technique »
>
> Étape « test → code ». Le test est le contrat, l'agent produit le code. Suit
> `docs/prompt/prompt-gabarit-code.md`.
>
> Prérequis : le test `tests/tests-unitaires/admin/case-admin-053.test.ts` doit exister et être rouge.
>
...
Commande pour ce seul test : npx vitest run --project admin tests/tests-unitaires/admin/case-admin-053.test.ts
Commande d'audit d'architecture : npm run arch:report

CE QUE JE TE DONNE
- Le test à faire passer : tests/tests-unitaires/admin/case-admin-053.test.ts::test_CASE_ADMIN_053_preremplissage_instantane_template_incident_technique
...
TA TÂCHE
1. Fais passer au vert le test test_CASE_ADMIN_053_preremplissage_instantane_template_incident_technique, et lui seul.
