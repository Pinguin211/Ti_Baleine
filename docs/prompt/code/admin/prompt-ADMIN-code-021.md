# Prompt — écrire le code de CASE-ADMIN-021

> Garantie de cohérence transactionnelle en cas de coupure réseau lors d'une annulation
>
> Étape « test → code ». Le test est le contrat, l'agent produit le code. Suit
> `docs/prompt/prompt-gabarit-code.md`.
>
> Prérequis : le test `tests/tests-unitaires/admin/case-admin-021.test.ts` doit exister et être rouge.
>
...
Commande pour ce seul test : npx vitest run --project admin tests/tests-unitaires/admin/case-admin-021.test.ts
Commande d'audit d'architecture : npm run arch:report

CE QUE JE TE DONNE
- Le test à faire passer : tests/tests-unitaires/admin/case-admin-021.test.ts::test_CASE_ADMIN_021_coherence_transactionnelle_annulation_rollback_reseau
...
TA TÂCHE
1. Fais passer au vert le test test_CASE_ADMIN_021_coherence_transactionnelle_annulation_rollback_reseau, et lui seul.
