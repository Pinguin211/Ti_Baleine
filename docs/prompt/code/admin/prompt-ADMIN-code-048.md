# Prompt — écrire le code de CASE-ADMIN-048

> Envoi groupé d'une alerte météo bilingue la veille à 18h sur plusieurs créneaux ciblés du lendemain
>
> Étape « test → code ». Le test est le contrat, l'agent produit le code. Suit
> `docs/prompt/prompt-gabarit-code.md`.
>
> Prérequis : le test `tests/tests-unitaires/admin/case-admin-048.test.ts` doit exister et être rouge.
>
...
Commande pour ce seul test : npx vitest run --project admin tests/tests-unitaires/admin/case-admin-048.test.ts
Commande d'audit d'architecture : npm run arch:report

CE QUE JE TE DONNE
- Le test à faire passer : tests/tests-unitaires/admin/case-admin-048.test.ts::test_CASE_ADMIN_048_envoi_groupe_alerte_meteo_veille_18h_multi_creneaux
...
TA TÂCHE
1. Fais passer au vert le test test_CASE_ADMIN_048_envoi_groupe_alerte_meteo_veille_18h_multi_creneaux, et lui seul.
