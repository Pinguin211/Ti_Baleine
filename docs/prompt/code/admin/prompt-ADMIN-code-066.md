# Prompt — écrire le code de CASE-ADMIN-066

> Blocage strict de toute tentative d'affecter deux activités différentes sur le même navire et même créneau
>
> Étape « test → code ». Le test est le contrat, l'agent produit le code. Suit
> `docs/prompt/prompt-gabarit-code.md`.
>
> Prérequis : le test `tests/tests-unitaires/admin/case-admin-066.test.ts` doit exister et être rouge.
>
...
Commande pour ce seul test : npx vitest run --project admin tests/tests-unitaires/admin/case-admin-066.test.ts
Commande d'audit d'architecture : npm run arch:report

CE QUE JE TE DONNE
- Le test à faire passer : tests/tests-unitaires/admin/case-admin-066.test.ts::test_CASE_ADMIN_066_blocage_mixite_activites_meme_navire_creneau_exclusivite
...
TA TÂCHE
1. Fais passer au vert le test test_CASE_ADMIN_066_blocage_mixite_activites_meme_navire_creneau_exclusivite, et lui seul.
