# Prompt — écrire le code de CASE-ADMIN-012

> Annulation standard d'une réservation hors alerte avec sélection d'un motif informatif et notification SMS
>
> Étape « test → code ». Le test est le contrat, l'agent produit le code. Suit
> `docs/prompt/prompt-gabarit-code.md`.
>
> Prérequis : le test `tests/tests-unitaires/admin/case-admin-012.test.ts` doit exister et être rouge.
>
...
Commande pour ce seul test : npx vitest run --project admin tests/tests-unitaires/admin/case-admin-012.test.ts
Commande d'audit d'architecture : npm run arch:report

CE QUE JE TE DONNE
- Le test à faire passer : tests/tests-unitaires/admin/case-admin-012.test.ts::test_CASE_ADMIN_012_annulation_standard_hors_alerte_motif_sms_client
...
TA TÂCHE
1. Fais passer au vert le test test_CASE_ADMIN_012_annulation_standard_hors_alerte_motif_sms_client, et lui seul.
