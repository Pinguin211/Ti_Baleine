# Prompt — écrire le code de CASE-ADMIN-018

> Rejet strict de la demande d'annulation administrative lorsque le créneau est déjà passé
>
> Étape « test → code ». Le test est le contrat, l'agent produit le code. Suit
> `docs/prompt/prompt-gabarit-code.md`.
>
> Prérequis : le test `tests/tests-unitaires/admin/case-admin-018.test.ts` doit exister et être rouge.
>
...
Commande pour ce seul test : npx vitest run --project admin tests/tests-unitaires/admin/case-admin-018.test.ts
Commande d'audit d'architecture : npm run arch:report

CE QUE JE TE DONNE
- Le test à faire passer : tests/tests-unitaires/admin/case-admin-018.test.ts::test_CASE_ADMIN_018_rejet_strict_annulation_administrative_creneau_passe
...
TA TÂCHE
1. Fais passer au vert le test test_CASE_ADMIN_018_rejet_strict_annulation_administrative_creneau_passe, et lui seul.
