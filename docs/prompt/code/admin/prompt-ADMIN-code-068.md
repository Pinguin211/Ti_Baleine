# Prompt — écrire le code de CASE-ADMIN-068

> Cloisonnement de sécurité et interdiction d'accès aux réglages de configuration pour les utilisateurs du site public
>
> Étape « test → code ». Le test est le contrat, l'agent produit le code. Suit
> `docs/prompt/prompt-gabarit-code.md`.
>
> Prérequis : le test `tests/tests-unitaires/admin/case-admin-068.test.ts` doit exister et être rouge.
>
...
Commande pour ce seul test : npx vitest run --project admin tests/tests-unitaires/admin/case-admin-068.test.ts
Commande d'audit d'architecture : npm run arch:report

CE QUE JE TE DONNE
- Le test à faire passer : tests/tests-unitaires/admin/case-admin-068.test.ts::test_CASE_ADMIN_068_cloisonnement_securite_interdiction_acces_configuration_creneaux_public
...
TA TÂCHE
1. Fais passer au vert le test test_CASE_ADMIN_068_cloisonnement_securite_interdiction_acces_configuration_creneaux_public, et lui seul.
