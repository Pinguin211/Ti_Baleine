# Prompt — écrire le code de CASE-ADMIN-078

> Bascule automatique vers « Payée complètement » après validation du webhook bancaire du solde en ligne
>
> Étape « test → code ». Le test est le contrat, l'agent produit le code. Suit
> `docs/prompt/prompt-gabarit-code.md`.
>
> Prérequis : le test `tests/tests-unitaires/admin/case-admin-078.test.ts` doit exister et être rouge.
>
...
Commande pour ce seul test : npx vitest run --project admin tests/tests-unitaires/admin/case-admin-078.test.ts
Commande d'audit d'architecture : npm run arch:report

CE QUE JE TE DONNE
- Le test à faire passer : tests/tests-unitaires/admin/case-admin-078.test.ts::test_CASE_ADMIN_078_bascule_payee_completement_webhook_bancaire_solde_en_ligne
- Le cas de test : tests/cases/admin/CASE-ADMIN-078.md
- La spécification : specs/admin.md, section SPEC-ADMIN-01
- Les exigences d'architecture : specs/architecture.md
- Les fichiers du domaine que tu peux modifier : src/admin/planning/

TA TÂCHE
1. Fais passer au vert le test test_CASE_ADMIN_078_bascule_payee_completement_webhook_bancaire_solde_en_ligne, et lui seul.
...
4. La règle vit dans le domaine : src/admin/planning/. Pas dans un contrôleur, pas dans un
...
7. Ne modifie aucun fichier en dehors de : `src/admin/planning/`.
