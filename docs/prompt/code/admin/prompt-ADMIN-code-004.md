# Prompt — écrire le code de CASE-ADMIN-004

> Affichage d'un état vide explicite lorsqu'aucun créneau n'est programmé
>
> Étape « test → code ». Le test est le contrat, l'agent produit le code. Suit
> `docs/prompt/prompt-gabarit-code.md`.
>
> Prérequis : le test `tests/tests-unitaires/admin/case-admin-004.test.ts` doit exister et être rouge.
>
...
Commande pour ce seul test : npx vitest run --project admin tests/tests-unitaires/admin/case-admin-004.test.ts
Commande d'audit d'architecture : npm run arch:report

CE QUE JE TE DONNE
- Le test à faire passer : tests/tests-unitaires/admin/case-admin-004.test.ts::test_CASE_ADMIN_004_affichage_etat_vide_explicite_aucun_creneau_programme
...
TA TÂCHE
1. Fais passer au vert le test test_CASE_ADMIN_004_affichage_etat_vide_explicite_aucun_creneau_programme, et lui seul.
