# Prompt — écrire le code de CASE-ADMIN-044

> Affichage d'un créneau à 0 billet actif avec mention explicite 0 % et 0 place réservée
>
> Étape « test → code ». Le test est le contrat, l'agent produit le code. Suit
> `docs/prompt/prompt-gabarit-code.md`.
>
> Prérequis : le test `tests/tests-unitaires/admin/case-admin-044.test.ts` doit exister et être rouge.
>
...
Commande pour ce seul test : npx vitest run --project admin tests/tests-unitaires/admin/case-admin-044.test.ts
Commande d'audit d'architecture : npm run arch:report

CE QUE JE TE DONNE
- Le test à faire passer : tests/tests-unitaires/admin/case-admin-044.test.ts::test_CASE_ADMIN_044_affichage_creneau_0_billet_actif_taux_0_pourcent
...
TA TÂCHE
1. Fais passer au vert le test test_CASE_ADMIN_044_affichage_creneau_0_billet_actif_taux_0_pourcent, et lui seul.
