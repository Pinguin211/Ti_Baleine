# Prompt — écrire le code de CASE-ADMIN-008

> Gestion d'une perte de connexion réseau pendant le chargement du planning
>
> Étape « test → code ». Le test est le contrat, l'agent produit le code. Suit
> `docs/prompt/prompt-gabarit-code.md`.
>
> Prérequis : le test `tests/tests-unitaires/admin/case-admin-008.test.ts` doit exister et être rouge.
>
...
Commande pour ce seul test : npx vitest run --project admin tests/tests-unitaires/admin/case-admin-008.test.ts
Commande d'audit d'architecture : npm run arch:report

CE QUE JE TE DONNE
- Le test à faire passer : tests/tests-unitaires/admin/case-admin-008.test.ts::test_CASE_ADMIN_008_gestion_perte_reseau_chargement_planning_erreur_retry
...
TA TÂCHE
1. Fais passer au vert le test test_CASE_ADMIN_008_gestion_perte_reseau_chargement_planning_erreur_retry, et lui seul.
