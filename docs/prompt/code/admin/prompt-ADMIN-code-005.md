# Prompt — écrire le code de CASE-ADMIN-005

> Affichage distinctif d'un créneau sans navire affecté avec le statut « non affecté »
>
> Étape « test → code ». Le test est le contrat, l'agent produit le code. Suit
> `docs/prompt/prompt-gabarit-code.md`.
>
> Prérequis : le test `tests/tests-unitaires/admin/case-admin-005.test.ts` doit exister et être rouge.
>
...
Commande pour ce seul test : npx vitest run --project admin tests/tests-unitaires/admin/case-admin-005.test.ts
Commande d'audit d'architecture : npm run arch:report

CE QUE JE TE DONNE
- Le test à faire passer : tests/tests-unitaires/admin/case-admin-005.test.ts::test_CASE_ADMIN_005_affichage_distinctif_creneau_sans_navire_non_affecte
...
TA TÂCHE
1. Fais passer au vert le test test_CASE_ADMIN_005_affichage_distinctif_creneau_sans_navire_non_affecte, et lui seul.
