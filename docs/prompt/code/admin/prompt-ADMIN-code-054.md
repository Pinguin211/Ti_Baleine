# Prompt — écrire le code de CASE-ADMIN-054

> Personnalisation et ajustement libre du texte/motif par l'administrateur dans le champ éditable
>
> Étape « test → code ». Le test est le contrat, l'agent produit le code. Suit
> `docs/prompt/prompt-gabarit-code.md`.
>
> Prérequis : le test `tests/tests-unitaires/admin/case-admin-054.test.ts` doit exister et être rouge.
>
...
Commande pour ce seul test : npx vitest run --project admin tests/tests-unitaires/admin/case-admin-054.test.ts
Commande d'audit d'architecture : npm run arch:report

CE QUE JE TE DONNE
- Le test à faire passer : tests/tests-unitaires/admin/case-admin-054.test.ts::test_CASE_ADMIN_054_personnalisation_ajustement_libre_texte_motif_avant_envoi
...
TA TÂCHE
1. Fais passer au vert le test test_CASE_ADMIN_054_personnalisation_ajustement_libre_texte_motif_avant_envoi, et lui seul.
