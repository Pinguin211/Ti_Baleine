# Prompt — écrire le code de CASE-ADMIN-016

> Annulation administrative autorisée sans délai minimal préalable jusqu'à l'heure exacte du départ
>
> Étape « test → code ». Le test est le contrat, l'agent produit le code. Suit
> `docs/prompt/prompt-gabarit-code.md`.
>
> Prérequis : le test `tests/tests-unitaires/admin/case-admin-016.test.ts` doit exister et être rouge.
>
...
Commande pour ce seul test : npx vitest run --project admin tests/tests-unitaires/admin/case-admin-016.test.ts
Commande d'audit d'architecture : npm run arch:report

CE QUE JE TE DONNE
- Le test à faire passer : tests/tests-unitaires/admin/case-admin-016.test.ts::test_CASE_ADMIN_016_annulation_administrative_autorisee_jusqua_heure_depart_h0
...
TA TÂCHE
1. Fais passer au vert le test test_CASE_ADMIN_016_annulation_administrative_autorisee_jusqua_heure_depart_h0, et lui seul.
