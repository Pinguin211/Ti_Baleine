# Prompt — écrire le code de CASE-ADMIN-039

> Expiration de session après un délai d'inactivité prolongée : déconnexion automatique
>
> Étape « test → code ». Le test est le contrat, l'agent produit le code. Suit
> `docs/prompt/prompt-gabarit-code.md`.
>
> Prérequis : le test `tests/tests-unitaires/admin/case-admin-039.test.ts` doit exister et être rouge.
>
...
Commande pour ce seul test : npx vitest run --project admin tests/tests-unitaires/admin/case-admin-039.test.ts
Commande d'audit d'architecture : npm run arch:report

CE QUE JE TE DONNE
- Le test à faire passer : tests/tests-unitaires/admin/case-admin-039.test.ts::test_CASE_ADMIN_039_expiration_session_inactivite_prolongee_deconnexion_auto
...
TA TÂCHE
1. Fais passer au vert le test test_CASE_ADMIN_039_expiration_session_inactivite_prolongee_deconnexion_auto, et lui seul.
