# Prompt — écrire le code de CASE-ADMIN-067

> Alerte et blocage de toute tentative de programmation simultanée de deux sorties Baleines nécessitant le naturaliste unique
>
> Étape « test → code ». Le test est le contrat, l'agent produit le code. Suit
> `docs/prompt/prompt-gabarit-code.md`.
>
> Prérequis : le test `tests/tests-unitaires/admin/case-admin-067.test.ts` doit exister et être rouge.
>
...
Commande pour ce seul test : npx vitest run --project admin tests/tests-unitaires/admin/case-admin-067.test.ts
Commande d'audit d'architecture : npm run arch:report

CE QUE JE TE DONNE
- Le test à faire passer : tests/tests-unitaires/admin/case-admin-067.test.ts::test_CASE_ADMIN_067_blocage_conflit_naturaliste_unique_deux_sorties_baleines_simultanees
...
- Les fichiers du domaine que tu peux modifier : src/actions/, src/services/server/slots/, src/schemas/validation/slots/

TA TÂCHE
1. Fais passer au vert le test test_CASE_ADMIN_067_blocage_conflit_naturaliste_unique_deux_sorties_baleines_simultanees, et lui seul.
...
4. La règle vit dans le domaine : src/services/server/slots/. Pas dans un contrôleur, pas dans un
...
8. Ne modifie aucun fichier en dehors de : `src/actions/`, `src/services/server/slots/`, `src/schemas/validation/slots/`.
