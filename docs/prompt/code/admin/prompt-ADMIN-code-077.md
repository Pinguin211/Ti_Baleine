# Prompt — écrire le code de CASE-ADMIN-077

> Affichage des statuts financiers des réservations sur le détail d'un créneau le jour J
>
> Étape « test → code ». Le test est le contrat, l'agent produit le code. Suit
> `docs/prompt/prompt-gabarit-code.md`.
>
> Prérequis : le test `tests/tests-unitaires/admin/case-admin-077.test.ts` doit exister et être rouge.
>
...
Commande pour ce seul test : npx vitest run --project admin tests/tests-unitaires/admin/case-admin-077.test.ts
Commande d'audit d'architecture : npm run arch:report

CE QUE JE TE DONNE
- Le test à faire passer : tests/tests-unitaires/admin/case-admin-077.test.ts::test_CASE_ADMIN_077_affichage_statuts_financiers_reservations_detail_creneau_jour_j
- Le cas de test : tests/cases/admin/CASE-ADMIN-077.md
...
TA TÂCHE
1. Fais passer au vert le test test_CASE_ADMIN_077_affichage_statuts_financiers_reservations_detail_creneau_jour_j, et lui seul.
