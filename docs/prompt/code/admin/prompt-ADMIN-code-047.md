# Prompt — écrire le code de CASE-ADMIN-047

> Affichage spécifique d'un créneau privatisé indiquant « Navire privatisé » et bloquant la totalité de la jauge
>
> Étape « test → code ». Le test est le contrat, l'agent produit le code. Suit
> `docs/prompt/prompt-gabarit-code.md`.
>
> Prérequis : le test `tests/tests-unitaires/admin/case-admin-047.test.ts` doit exister et être rouge.
>
...
Commande pour ce seul test : npx vitest run --project admin tests/tests-unitaires/admin/case-admin-047.test.ts
Commande d'audit d'architecture : npm run arch:report

CE QUE JE TE DONNE
- Le test à faire passer : tests/tests-unitaires/admin/case-admin-047.test.ts::test_CASE_ADMIN_047_affichage_specifique_creneau_privatise_blocage_jauge
...
TA TÂCHE
1. Fais passer au vert le test test_CASE_ADMIN_047_affichage_specifique_creneau_privatise_blocage_jauge, et lui seul.
