# Prompt — écrire le code de CASE-ADMIN-015

> Remise à disposition immédiate et synchrone de la totalité des places libérées sur l'interface publique
>
> Étape « test → code ». Le test est le contrat, l'agent produit le code. Suit
> `docs/prompt/prompt-gabarit-code.md`.
>
> Prérequis : le test `tests/tests-unitaires/admin/case-admin-015.test.ts` doit exister et être rouge.
>
...
Commande pour ce seul test : npx vitest run --project admin tests/tests-unitaires/admin/case-admin-015.test.ts
Commande d'audit d'architecture : npm run arch:report

CE QUE JE TE DONNE
- Le test à faire passer : tests/tests-unitaires/admin/case-admin-015.test.ts::test_CASE_ADMIN_015_remise_a_disposition_immediate_places_interface_publique
...
TA TÂCHE
1. Fais passer au vert le test test_CASE_ADMIN_015_remise_a_disposition_immediate_places_interface_publique, et lui seul.
