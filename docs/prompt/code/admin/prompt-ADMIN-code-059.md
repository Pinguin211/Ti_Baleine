# Prompt — écrire le code de CASE-ADMIN-059

> Déclenchement d'une alerte sur un créneau sans aucune réservation : passage sous pré-alerte sans message sortant
>
> Étape « test → code ». Le test est le contrat, l'agent produit le code. Suit
> `docs/prompt/prompt-gabarit-code.md`.
>
> Prérequis : le test `tests/tests-unitaires/admin/case-admin-059.test.ts` doit exister et être rouge.
>
...
Commande pour ce seul test : npx vitest run --project admin tests/tests-unitaires/admin/case-admin-059.test.ts
Commande d'audit d'architecture : npm run arch:report

CE QUE JE TE DONNE
- Le test à faire passer : tests/tests-unitaires/admin/case-admin-059.test.ts::test_CASE_ADMIN_059_alerte_creneau_sans_reservation_statut_pre_alerte_sans_message
...
TA TÂCHE
1. Fais passer au vert le test test_CASE_ADMIN_059_alerte_creneau_sans_reservation_statut_pre_alerte_sans_message, et lui seul.
