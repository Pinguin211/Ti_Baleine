# Prompt — écrire le code de CASE-ADMIN-013

> Non-persistance du motif d'annulation sur l'entité réservation en base
>
> Étape « test → code ». Le test est le contrat, l'agent produit le code. Suit
> `docs/prompt/prompt-gabarit-code.md`.
>
> Prérequis : le test `tests/tests-unitaires/admin/case-admin-013.test.ts` doit exister et être rouge.
>
...
Commande pour ce seul test : npx vitest run --project admin tests/tests-unitaires/admin/case-admin-013.test.ts
Commande d'audit d'architecture : npm run arch:report

CE QUE JE TE DONNE
- Le test à faire passer : tests/tests-unitaires/admin/case-admin-013.test.ts::test_CASE_ADMIN_013_non_persistance_motif_annulation_table_bookings
...
TA TÂCHE
1. Fais passer au vert le test test_CASE_ADMIN_013_non_persistance_motif_annulation_table_bookings, et lui seul.
