# Prompt — écrire le code de CASE-ADMIN-080

> Perte temporaire de connexion internet lors de la validation du pointage et de l'encaissement du solde sur place
>
> Étape « test → code ». Le test est le contrat, l'agent produit le code. Suit
> `docs/prompt/prompt-gabarit-code.md`.
>
> Prérequis : le test `tests/tests-unitaires/admin/case-admin-080.test.ts` doit exister et être rouge.
>
...
Commande pour ce seul test : npx vitest run --project admin tests/tests-unitaires/admin/case-admin-080.test.ts
Commande d'audit d'architecture : npm run arch:report

CE QUE JE TE DONNE
- Le test à faire passer : tests/tests-unitaires/admin/case-admin-080.test.ts::test_CASE_ADMIN_080_perte_connexion_pointage_encaissement_solde_sur_place
- Le cas de test : tests/cases/admin/CASE-ADMIN-080.md
...
TA TÂCHE
1. Fais passer au vert le test test_CASE_ADMIN_080_perte_connexion_pointage_encaissement_solde_sur_place, et lui seul.
