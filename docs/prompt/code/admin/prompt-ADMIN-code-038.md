# Prompt — écrire le code de CASE-ADMIN-038

> Protection anti-bruteforce : ralentissement et blocage temporaire après tentatives répétées
>
> Étape « test → code ». Le test est le contrat, l'agent produit le code. Suit
> `docs/prompt/prompt-gabarit-code.md`.
>
> Prérequis : le test `tests/tests-unitaires/admin/case-admin-038.test.ts` doit exister et être rouge.
>
...
Commande pour ce seul test : npx vitest run --project admin tests/tests-unitaires/admin/case-admin-038.test.ts
Commande d'audit d'architecture : npm run arch:report

CE QUE JE TE DONNE
- Le test à faire passer : tests/tests-unitaires/admin/case-admin-038.test.ts::test_CASE_ADMIN_038_protection_anti_bruteforce_blocage_temporaire_tentatives_repetees
...
TA TÂCHE
1. Fais passer au vert le test test_CASE_ADMIN_038_protection_anti_bruteforce_blocage_temporaire_tentatives_repetees, et lui seul.
