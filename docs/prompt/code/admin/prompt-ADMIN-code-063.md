# Prompt — écrire le code de CASE-ADMIN-063

> Réouverture manuelle exceptionnelle d'un créneau précédemment fermé depuis le tableau de bord
>
> Étape « test → code ». Le test est le contrat, l'agent produit le code. Suit
> `docs/prompt/prompt-gabarit-code.md`.
>
> Prérequis : le test `tests/tests-unitaires/admin/case-admin-063.test.ts` doit exister et être rouge.
>
...
Commande pour ce seul test : npx vitest run --project admin tests/tests-unitaires/admin/case-admin-063.test.ts
Commande d'audit d'architecture : npm run arch:report

CE QUE JE TE DONNE
- Le test à faire passer : tests/tests-unitaires/admin/case-admin-063.test.ts::test_CASE_ADMIN_063_reouverture_manuelle_exceptionnelle_creneau_ferme
...
TA TÂCHE
1. Fais passer au vert le test test_CASE_ADMIN_063_reouverture_manuelle_exceptionnelle_creneau_ferme, et lui seul.
