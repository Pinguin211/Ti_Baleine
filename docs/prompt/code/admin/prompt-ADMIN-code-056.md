# Prompt — écrire le code de CASE-ADMIN-056

> Basculement automatique du statut des créneaux ciblés à « sous pré-alerte » dès confirmation d'envoi
>
> Étape « test → code ». Le test est le contrat, l'agent produit le code. Suit
> `docs/prompt/prompt-gabarit-code.md`.
>
> Prérequis : le test `tests/tests-unitaires/admin/case-admin-056.test.ts` doit exister et être rouge.
>
...
Commande pour ce seul test : npx vitest run --project admin tests/tests-unitaires/admin/case-admin-056.test.ts
Commande d'audit d'architecture : npm run arch:report

CE QUE JE TE DONNE
- Le test à faire passer : tests/tests-unitaires/admin/case-admin-056.test.ts::test_CASE_ADMIN_056_basculement_automatique_statut_creneau_sous_pre_alerte
...
TA TÂCHE
1. Fais passer au vert le test test_CASE_ADMIN_056_basculement_automatique_statut_creneau_sous_pre_alerte, et lui seul.
