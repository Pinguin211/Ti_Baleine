...
|---|---|---|
| Q01 | Préfères-tu une solution SaaS clé-en-main (ex. Bokun, Resagenda) avec abonnement/commission, ou un développement sur-mesure qui t’appartient ? | |
| Q02 | Quels types d’activités proposes-tu ? (excursions, observation baleines, plongée, privatisation, etc.) | |
| Q03 | Quelles formules/produits existent (découverte, premium, groupe) et quelles sont les durées associées ? | |
| Q04 | Le système doit-il être intégré à un site web existant ou développé comme une plateforme autonome ? | |
...
| Q07 | L’interface doit-elle être multilingue dès le lancement ? | |
| Q08 | Si multilingue : quelles langues (FR, EN, DE, …) et qui fournit les traductions ? | |
| Q09 | Combien de départs par jour / semaine proposez-vous ? | |
| Q10 | Les créneaux horaires sont-ils fixes ou variables selon la saison ? | |
| Q11 | Quel est le délai minimum de réservation avant le départ (ex. 2 h, 24 h, 48 h) ? | |
| Q12 | Quelle est la capacité minimale et maximale par sortie (places, bateau, véhicule) ? | |
...
| Q15 | Combien de bateaux / ressources avez-vous à gérer ? | |
| Q16 | Faut-il affecter un bateau / capitaine précis à chaque créneau ? | |
| Q17 | Le calendrier doit-il être synchronisé avec des agendas externes (Google Calendar, Apple, Outlook) ? | |
| Q18 | Quels tarifs proposez-vous (adulte, enfant, bébé, résident, privatisation) ? | |
...
| Q24 | Les clients doivent-ils payer en ligne lors de la réservation ? | |
| Q25 | Exigez-vous 100 % du paiement à la réservation ou un acompte ? | |
...
| Q28 | Quels moyens de paiement accepter en ligne (CB, Stripe, PayPal, Apple Pay, Google Pay, Chèques Vacances Connect) ? | |
...
| Q31 | Quel est le barème / délai et le pourcentage de remboursement selon le moment de l’annulation par le client (ex. 100 % si > 48 h, 50 % entre 48 h et 24 h, 0 % si < 24 h) ? | |
...
| Q35 | Combien de temps avant le départ cette décision est-elle prise (veille, H-2) ? | |
| Q36 | En cas d’annulation par le prestataire (météo/panne), comment gérer les clients en masse (lien de re-choix de date en autonomie, avoir automatique, remboursement direct) ? | |
...
| Q47 | Le site doit-il générer automatiquement une facture PDF à la validation de la commande ? | |
