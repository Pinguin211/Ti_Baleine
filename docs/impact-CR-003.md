...
| 13 | Quels sont les horaires des départs de Saint-Leu et quel est l'impact sur Saint-Gilles ? | **Départ à 9 h** de Saint-Leu (Tikap). Ensuite le bateau repart se positionner à Saint-Gilles : **pas de Tikap disponible à 10 h le mardi et le jeudi à Saint-Gilles** (jauge 10h à Saint-Gilles = 24 places sur Grand Bleu seul). |
...
| REQ-003 *(Visualisation créneaux disponibles)* | modifiée | Afficher les créneaux par site : Saint-Gilles (7h, 10h, 14h toute la semaine) et Saint-Leu (mardi et jeudi à 9h00 uniquement, Tikap 12 places). Brider la capacité de 10h00 à Saint-Gilles à 24 places le mardi et le jeudi. |
| REQ-004 *(Tarification et tranches d'âge)* | modifiée | Appliquer la majoration géographique de **+ 10 € par personne** pour les billets individuels au départ de Saint-Leu (Baleines 75 € ad / 50 € enf ; Dauphins 60 € ad / 40 € enf). Maintenir le forfait de privatisation Tikap à 600 € à Saint-Leu. |
...
| REQ-010 *(Visualisation jauges et remplissage)* | modifiée | Intégrer dans le tableau de bord admin la jauge spécifique de 24 places le mardi/jeudi à 10h à Saint-Gilles (Grand Bleu seul, absence du Tikap). |
...
| R-01 *(Planning standard)* | modifiée | Intégrer l'exception du créneau de 9h00 à Saint-Leu les mardis et jeudis pour le Tikap, et l'indisponibilité du Tikap à 10h00 ces mêmes jours à Saint-Gilles. |
| R-03 / R-09 *(Flotte et jauges)* | modifiée | Fixer la jauge maximale du mardi et jeudi à 10h00 à Saint-Gilles à **24 places** (Grand Bleu uniquement). |
...
| R-15 / CDC §6 *(Périmètre annulation)* | modifiée | Confirmer que le client ne peut pas annuler en ligne de manière autonome. L'annulation est un outil d'administration back-office, avec SMS d'information client et libération de créneau. Remboursements financiers maintenus hors système. |
...
|---|---|---|
| SPEC-RES-01 *(Tunnel de réservation client)* | modifiée | Choix du port d'embarquement (Saint-Gilles / Saint-Leu), affichage du créneau de 9h00 le mar/jeu pour Saint-Leu, application de la majoration de 10 €/pers. sur billets individuels, et jauge max à 24 places le mar/jeu à 10h à Saint-Gilles. |
...
| SPEC-ANNUL-01 *(Annulation back-office administrateur)* | ajoutée *(nouvelle)* | Écran et bouton d'action d'annulation dans l'interface admin : confirmation, mise à jour du statut en `ANNULEE`, libération immédiate des places associées dans la jauge du créneau, et déclenchement d'un appel à la passerelle SMS. Pas d'accès public/client. |
| SPEC-ADM-02 *(Planning & Supervision back-office)* | modifiée | Planning filtrable par port (Saint-Gilles / Saint-Leu), prise en compte de la jauge réduite à 24 places le mar/jeu à 10h à Saint-Gilles, et suivi de l'affectation du naturaliste unique. |
...
- **Oui (données nouvelles) :** Notion de lieu/port de départ (`Saint-Gilles`, `Saint-Leu`), surcoût tarifaire géographique (+ 10 € / pers sur billets individuels), créneaux horaires spécifiques (9h00 à St-Leu, 7h–12h en privatisation matin), numéro de mobile client validé pour SMS.
...
| CASE-RES-03 *(Blocage jauge max 36 places à Saint-Gilles hors mar/jeu 10h)* | inchangé |
...
| CASE-RES-07bis *(Contrainte Saint-Gilles mardi/jeudi 10h : blocage de la jauge à 24 places Grand Bleu seul)* | à écrire *(nouveau)* |
...
| Composant | Impact |
|---|---|
| `BookingEngine / SlotEngine` | Intégrer les règles multi-sites : Saint-Leu le mar/jeu à 9h (12 places Tikap) ; jauge Saint-Gilles le mar/jeu à 10h bridée à 24 places (Grand Bleu seul) ; créneaux de privatisation matin (7h–12h). |
...
| `AdminPlanningService` | Adapter le tableau de bord avec filtres par lieu et visualisation des capacités réelles (24 places le mar/jeu 10h à Saint-Gilles). |
...
- **Jauge réduite à Saint-Gilles le mardi et le jeudi à 10h :** Le *Tikap* étant engagé à Saint-Leu à 9h00 (retour à quai ~11h-11h30 + transit maritime), il ne peut pas assurer le créneau de 10h00 à Saint-Gilles. La capacité du créneau de 10h00 à Saint-Gilles ces jours-là doit être automatiquement plafonnée à **24 places** par le moteur de réservation (Grand Bleu uniquement).
...
- **Ressource critique du naturaliste unique :** Si une sortie baleines est programmée à 9h00 à Saint-Leu, le naturaliste y est mobilisé ; aucune sortie baleines ne peut avoir lieu en parallèle le matin à Saint-Gilles.
...
| # | Étape | Qui |
|---|---|---|
| 1 | Mise à jour du Cahier des Charges Fonctionnel (`cahier-des-charges-v3.md` : intégration de Saint-Leu, jauge mar/jeu 10h à 24 places, privatisations St-Leu 600 €, annulation/modification admin avec SMS client) | Thomas & Loïc |
...
| 4 | Mise à jour du MCD / MLD / MPD (table `Lieux`, jauges conditionnelles, statuts de réservation) | Benjamin |
...
| 6 | Développement du moteur de réservation multi-sites, de la règle mar/jeu 10h (24 places) et de la majoration (+ 10 €) | Toute l'équipe |
