...
| REQ-020 *(Saisie du motif d'annulation pour SMS client)* | ajoutée *(nouvelle)* | Proposer à l'administrateur la saisie ou la sélection d'un motif (annulation météo/créneau, annulation « par peur » du client suite à l'alerte, ou annulation standard) pour composer le SMS de notification au client ; le motif n'est pas persisté en base (les remboursements restant 100 % manuels hors système). |
